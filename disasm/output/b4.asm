...
OW_Tile_Animation:  LDA $13                   ; \ ; Index (8 bit) Accum (8 bit) 
                    AND.B #$07                ;  |If lower 3 bits of frame counter isn't 0,
                    BNE ADDR_048101           ; / don't update the water animation
                    LDX.B #$1F                
...
ADDR_048101:        LDA $13                   ; \
                    AND.B #$07                ;  |If lower 3 bits of frame counter isn't 0,
                    BNE ADDR_04810C           ; / don't update the waterfall animation
                    LDX.B #$20                
                    JSR.W ADDR_048172         
ADDR_04810C:        LDA $13                   ; \
                    AND.B #$07                ;  |If lower 3 bits of frame counter isn't 0,
                    BNE ADDR_048123           ; / branch to $8123
                    LDX.B #$1F                
...
GameMode_0E_Prim:   PHB                       
                    PHK                       
                    PLB                       
                    LDX.B #$01                ;  \ If player 1 pushes select...
ADDR_048246:        LDA.W $0DA6,X             ;   |
                    AND.B #$20                ;   | ...disabled by BRA
                    BRA ADDR_048261           ;  / Change to BEQ to enable debug code below

                    LDA.W $0DBA,X             ; \ Unreachable
...
ADDR_048261:        DEX                       
                    BPL ADDR_048246           
                    JSR.W ADDR_0485A7         
                    JSR.W OW_Tile_Animation   
                    LDA.W $13D2               ; \ If "! blocks flying away color" is 0,
                    BEQ ADDR_048275           ; / don't play the animation
                    JSR.W ADDR_04F290         
                    JMP.W ADDR_04840D         

ADDR_048275:        LDA.W $13C9               ; \ If not showing Continue/End message,
                    BEQ ADDR_048281           ; / branch to $8281
                    JSL.L ExtSub009B80        
                    JMP.W ADDR_048410         
...
ADDR_048375:        LDX.W $0DB3               
                    ...
                    AND.B #$10                
                    BEQ ADDR_04839A           
                    INC.W $13D4               ;  Look around overworld
                    LDA.W $13D4               
                    LSR                       
                    BNE ADDR_04839A           
                    REP #$20                  ; Accum (16 bit) 
...
ADDR_048509:        LDY.W $0DB3               ; \ Get current player's submap
                    LDA.W $1F11,Y             ; /
                    STA $01                   ; Store it in $01
                    STZ $00                   ; Store x00 in $00
                    REP #$20                  ; 16 bit A ; Accum (16 bit) 
                    LDX.W $0DD6               ; Set X to Current character*4
...
ADDR_048531:        DEY                       
                    DEY                       
                    BPL ADDR_04851A           
ADDR_048535:        STY.W $1DF6               ; Store Y in "Warp destination"
...
DrawOWBoarder?:     JSR.W ADDR_04862E         
...
ADDR_04862E:        REP #$30                  ; Index (16 bit) Accum (16 bit) 
                    LDX.W $0DD6               ; \ Get current player's X position
                    LDA.W $1F17,X             ; /
                    SEC                       ; \ Subtract x1A
                    SBC $1A                   ; /
                    CMP.W #$0100              ; \ If A >= x100,
                    BCS ADDR_04864D           ; / branch to $864D
                    STA $00                   ; \ Store in $00 and $08
                    STA $08                   ; /
                    LDA.W $1F19,X             
                    SEC                       
                    SBC $1C                   
...
ADDR_048EE1:        REP #$30                  ; Index (16 bit) Accum (16 bit) 
                    ...
                    JSR.W DrawOWBoarder?      
                    JSR.W ADDR_048086         
                    JMP.W OW_Tile_Animation   

ADDR_048EF1:        LDA.B #$08                
                    STA.W $0DB1               
                    LDA.W $1F11               
                    CMP.B #$01                
                    BNE ADDR_048F13           
...
ADDR_048F87:        JSR.W ADDR_049903         ; Index (8 bit) 
                    LDX.B #$07                
...
ADDR_049132:        LDA $16                   
                    AND.B #$20                
                    BRA OW_Player_Update      ; Change to BEQ to enable below debug code

                    LDA.W $13C1               ; \ Unreachable
                    BEQ ADDR_049165           ;  | Debug: Warp to star road from Yoshi's house
                    CMP.B #$56                ;  |
                    BEQ ADDR_049165           ; /
OW_Player_Update:   LDA $17                   ; \
...
OWPU_NoLR:          LDA $16                   ; \
                    ORA $18                   ;  |If A, B, X or Y are pressed,
                    AND.B #$C0                ;  |branch to OWPU_ABXY
                    BNE OWPU_ABXY             ;  |Otherwise,
                    BRL ADDR_0491E9           ; / branch to $91E9
OWPU_ABXY:          STZ.W $1B9E               
                    LDA.W $13C1               ; \
...
ADDR_049165:        JSR.W ADDR_048509         
                    ...
                    STZ.W $1DF7               ; Set "Fly away" speed to 0
                    STZ.W $1DF8               ; Set "Stay on ground" timer to 0 (31 = Fly away)
                    LDA.B #$0D                ; \ Star Road sound effect
                    STA.W $1DF9               ; /
                    LDA.B #$0B                ; \ Activate star warp
                    STA.W $13D9               ; /
                    JMP.W ADDR_049E52         

OWPU_NotOnStar:     LDA.W $13C1               ; \
...
OWPU_IsOnPipe:      JSR.W ADDR_048509         
                    BNE OWPU_IsOnPipeRTS      
ADDR_04918D:        INC.W $1B9C               
                    STZ.W $0DD5               ; Set auto-walk to 0
                    LDA.B #$0B                ; \ Fade to overworld
                    STA.W $0100               ; /
OWPU_IsOnPipeRTS:   RTS                       ; Return

OWPU_NotOnPipe:     CMP.B #$81                ; \
                    BEQ ADDR_0491E9           ;  |If standing on a tile >= (?) Destroyed Castle,
                    BCS ADDR_0491E9           ; / branch to $91E9
OWPU_EnterLevel:    LDA.W $0DD6               ; \
                    ...
                    BEQ ADDR_0491B1           ;  |Otherwise, use "raise hand, swimming" animation
                    LDY.B #$12                ;  |
ADDR_0491B1:        TYA                       ;  |
                    STA.W $1F13,X             ; /
                    LDX.W $0DB3               ; Get current character
                    LDA.W $0DB6,X             ; \ Get character's coins
                    STA.W $0DBF               ; /
                    LDA.W $0DB4,X             ; \ Get character's lives
                    STA.W $0DBE               ; /
                    LDA.W $0DB8,X             ; \ Get character's powerup
                    STA $19                   ; /
                    LDA.W $0DBA,X             ; \
                    STA.W $0DC1               ;  |Get character's Yoshi color
                    STA.W $13C7               ;  |
                    STA.W $187A               ; /
                    LDA.W $0DBC,X             ; \ Get character's reserved item
                    STA.W $0DC2               ; /
                    LDA.B #$02                ; \ Related to fade speed
                    STA.W $0DB1               ; /
                    LDA.B #$80                ; \ Music fade out
                    STA.W $1DFB               ; / 
                    INC.W $0100               ; Fade to level
                    RTS                       ; Return

ADDR_0491E9:        REP #$20                  ; 16 bit A ; Accum (16 bit) 
                    LDX.W $0DD6               ; Get current character * 4
                    LDA.W $1F17,X             ; Get character's X coordinate
                    LSR                       ; \
                    LSR                       ;  |Divide X coordinate by 16
                    LSR                       ;  |
                    LSR                       ; /
                    STA $00                   ; \ Store in $00 and $1F1F,x
                    STA.W $1F1F,X             ; /
                    LDA.W $1F19,X             ; Get character's Y coordinate
                    LSR                       ; \
                    LSR                       ;  |Divide Y coordinate by 16
                    LSR                       ;  |
                    LSR                       ; /
                    STA $02                   ; \ Store in $02 and $1F21,x
                    STA.W $1F21,X             ; /
                    TXA                       ; \
                    LSR                       ;  |Divide (current character * 4) by 4
                    LSR                       ;  |
                    TAX                       ; /
                    JSR.W OW_TilePos_Calc     ; Calculate current tile pos
                    SEP #$20                  ; 8 bit A ; Accum (8 bit) 
                    LDX.W $0DD5               ; \ If auto-walk=0,
                    ...
                    LDX $04                   ; X = tile pos
                    LDA.L $7ED000,X           ; \ Get level number of current tile pos
                    AND.W #$00FF              ; /
                    LDY.W #$000A              
...
OWPU_NotAutoWalk:   SEP #$30                  ; 8 bit A,X,Y ; Index (8 bit) Accum (8 bit) 
                    STZ.W $0DD5               ; Set auto-walk to 0
                    LDA $16                   ; \
                    AND.B #$0F                ;  |If no dir button is pressed (one frame),
                    BEQ ADDR_04926E           ; / branch to $926E
                    LDX.W $13C1               ; \
                    CPX.B #$82                ;  |If standing on Pipe#2,
                    BEQ ADDR_0492AD           ;  |branch to $92AD
                    BRA ADDR_04928C           ; / Otherwise, branch to $928C

ADDR_04926E:        DEC.W $144E               ; \ Decrease "Face walking dir" timer
                    BPL ADDR_049287           ; / If >= 0, branch to $9287
                    STZ.W $144E               ; Set "Face walking dir" timer to 0
                    LDA.W $0DD6               ; \
                    ...
                    STA.W $1F13,X             ; / is in water or not.
...
ADDR_049301:        STZ.W $1B78               
                    ...
                    STA $00                   
                    LDX.W #$0009              
ADDR_049315:        LDA.W HardCodedOWPaths,X  
                    AND.W #$00FF              
                    CMP.W #$00FF              
                    BNE ADDR_049349           
                    PHX                       
...
ADDR_0495CE:        LDX $04                   
                    ...
                    CMP.W #$0800              
                    BCS ADDR_049575           
                    LDA.L $7EC800,X           ; \ Load OW tile number
                    AND.W #$00FF              ; /
ADDR_0495DE:        STA.W $13C1               ; Set "Current OW tile"
                    BEQ ADDR_049575           
                    CMP.W #$0087              
                    BCS ADDR_049575           
                    PHA                       
...
OW_TilePos_Calc:    LDA $00                   ; Get overworld X pos/16 (X) ; Accum (16 bit) 
                    AND.W #$000F              ; \
                    STA $04                   ;  |
                    LDA $00                   ;  |
                    AND.W #$0010              ;  |
                    ...
                    ADC $04                   ;  |
                    STA $04                   ; /
                    LDA $02                   ; Get overworld Y pos/16 (Y)
                    ASL                       ; \
                    ASL                       ;  |
                    ...
                    LDA $02                   ; \
                    AND.W #$0010              ;  |
                    BEQ ADDR_0498B5           ;  |If (Y&0x10) isn't 0,
                    LDA $04                   ;  |increase tile pos by x200
                    CLC                       ;  |
                    ADC.W #$0200              ;  |
                    STA $04                   ; /
ADDR_0498B5:        LDA.W $1F11,X             ; \
                    ...
                    LDA $04                   ;  |Increase tile pos by x400
                    CLC                       ;  |
                    ADC.W #$0400              ;  |
                    STA $04                   ; /
Return0498C5:       RTS                       ; Return

ADDR_0498C6:        STZ.W $1F13               ; Accum (8 bit) 
                    LDA.B #$80                
                    CLC                       
                    ADC.W $13D7               
                    STA.W $13D7               
...
ADDR_049D7F:        LDA.W LevelNameStrings,Y  ; Index (8 bit) Accum (8 bit) 
                    PHP                       
                    CPX $02                   
                    BCS ADDR_049D95           
                    AND.B #$7F                
...
ADDR_049DD1:        LDA.W $0DB3               ; Index (8 bit) Accum (8 bit) 
                    EOR.B #$01                
                    STA.W $0DB3               
                    TAX                       
                    LDA.W $0DB6,X             
...
ADDR_049E22:        DEC.W $0DB1               
                    ...
                    EOR.B #$01                
                    STA.W $0DAF               
Return049E4B:       RTS                       ; Return

ADDR_049E4C:        LDA.B #$03                
                    STA.W $13D9               
                    RTS                       ; Return
...
ADDR_049E9D:        CMP.B #$0A                ;  | While A >= #$0A...
                    BCC Return049EA6          ;  |
                    SBC.B #$0A                ;  | A -= #$0A
                    INY                       ;  | Y++
                    BRA ADDR_049E9D           ; /

Return049EA6:       RTS                       ; / Return
...
ExtSub04D6E9:       REP #$30                  ; Index (16 bit) Accum (16 bit) 
                    STZ $1C                   
                    LDA.W #$FFFF              
                    STA $4D                   
                    STA $4F                   
                    ...
                    LDA.W #$0200              
                    STA $1C                   
ADDR_04D714:        JSL.L ExtSub05881A        
                    JSL.L ExtSub0087AD        
                    REP #$30                  ; Index (16 bit) Accum (16 bit) 
                    INC $47                   
                    LDA $1C                   
...
ADDR_04DAEF:        SEP #$30                  ; Index (8 bit) Accum (8 bit) 
                    LDA.W $1DE8               
                    JSL.L ExecutePtr          
...
ADDR_04DBF3:        LDA.W $1F11,Y             
                    ...
                    STA.W $1DFB               ; / Change music
                    STZ.W $1B9E               
Return04DC01:       RTS                       ; Return

DATA_04DC02:        .db $11,$12,$13,$14,$15,$16,$17

ExtSub04DC09:       SEP #$30                  ; Index (8 bit) Accum (8 bit) 
                    LDA.W $0DD6               
                    LSR                       
                    LSR                       
                    TAX                       
...
ADDR_04F3FA:        JSL.L ExtSub009BA8        
                    RTS                       ; Return
...
ADDR_04F415:        LDX.B #$00                
                    LDA.W $0DB4               
                    CMP.W $0DB5               
                    BPL ADDR_04F420           
                    INX                       
ADDR_04F420:        STX.W $1B8A               
                    LDX.W $1B88               
                    LDA.W $1B89               
                    CMP.L DATA_04F413,X       
                    BNE ADDR_04F44B           
...
ExtSub04F675:       PHB                       
                    ...
                    LDX.B #$0C                
                    LDY.B #$4B                
ADDR_04F67C:        LDA.W DATA_04F625-15,Y    
                    STA.W $0DE8,X             
                    CMP.B #$01                
                    BEQ ADDR_04F68A           
                    CMP.B #$02                
...
ADDR_04F68A:        LDA.B #$40                
                    STA.W $0E58,X             
ADDR_04F68F:        LDA.W DATA_04F625-14,Y    
                    STA.W $0E38,X             
                    LDA.W DATA_04F625-13,Y    
                    STA.W $0E68,X             
                    LDA.W DATA_04F625-12,Y    
                    STA.W $0E48,X             
                    LDA.W DATA_04F625-11,Y    
                    STA.W $0E78,X             
                    TYA                       
                    SEC                       
                    SBC.B #$05                
...
ADDR_04FAF1:        JSR.W ADDR_04FED7         
                    JSR.W ADDR_04FE62         ; NOP this and the sprite doesn't appear
                    JSR.W ADDR_04FE5B         ; NOP this and the sprite stops animating.
                    LDY.B #$2A                ; Tile for pirahna plant, #1
                    AND.B #$08                
                    BEQ ADDR_04FB02           
                    LDY.B #$2C                ; Tile for pirahna plant, #2, stored in $0242
ADDR_04FB02:        LDA.B #$32                ; YXPPCCCT - 00110010
                    XBA                       
                    TYA                       
ADDR_04FB06:        SEC                       
                    LDY.W DATA_04F843,X       
ADDR_04FB0A:        STA.W $0242,Y             ; Tilemap
                    XBA                       
                    STA.W $0243,Y             ; Property
                    LDA $01                   
                    BNE Return04FB36          
                    LDA $00                   
                    STA.W $0240,Y             ; X Position
                    LDA $03                   
                    BNE Return04FB36          
                    PHP                       
                    LDA $02                   
                    STA.W $0241,Y             ; Y Position
                    TYA                       
                    LSR                       
                    LSR                       
                    PLP                       
                    ...
                    DEY                       
                    DEY                       
Return04FB36:       RTS                       ; Return

ADDR_04FB37:        LDA.B #$02                ; \Overworld Sprite X Speed
                    STA.W $0E95,X             ; /
                    LDA.B #$FF                ; \Overworld Sprite Y Speed
                    STA.W $0EA5,X             ; /
                    JSR.W ADDR_04FE90         ; Move the overworld cloud
                    JSR.W ADDR_04FE62         
                    REP #$20                  ; Accum (16 bit) 
                    LDA $00                   
                    CLC                       
...
ADDR_04FCE1:        JSR.W ADDR_04FE62         ; Bowser's sign code starts here.
...
ADDR_04FCEF:        LDA $13                   
                    ...
                    XBA                       
                    LDA $05                   
                    JSR.W ADDR_04FAED         ; Jump to CLC, then the OAM part of the Pirahna Plant code.
                    LDA $00                   
                    SEC                       
                    SBC.B #$08                
                    STA $00                   
...
ADDR_04FE90:        TXA                       ; Transfer X to A
                    CLC                       ; Clear Carry Flag
                    ADC.B #$20                ; Add #$20 to A
                    TAX                       ; Transfer A to X
                    JSR.W ADDR_04FEAB         
                    LDA.W $0E35,X             ; Load OW Sprite XPos Low
                    BPL ADDR_04FEA0           ; If it is => 80
                    STZ.W $0E35,X             ; Store 00 OW Sprite Xpos Low
ADDR_04FEA0:        TXA                       ; Transfer X to A
                    SEC                       ; Set Carry Flag...
                    SBC.B #$10                ; ...for substraction
                    TAX                       ; Transfer A to X
                    JSR.W ADDR_04FEAB         
                    LDX.W $0DDE               
ADDR_04FEAB:        LDA.W $0E95,X             ; Load OW Sprite X Speed
                    ...
                    CLC                       ; Clear Carry Flag
                    ADC.W $0EC5,X             
                    STA.W $0EC5,X             ; And store it in
                    LDA.W $0E95,X             ; Load OW Sprite X Speed
                    PHP                       
                    LSR                       ; Divide by 2
                    LSR                       ; 4
                    LSR                       ; 8
